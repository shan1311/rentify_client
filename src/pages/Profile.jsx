...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://rentify-server-iaby.onrender.com/api/user/update/${currentUser._id}`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const res = await fetch(`https://rentify-server-iaby.onrender.com/api/user/delete/${currentUser._id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
  ...
  const handleSignOut = async () => {
    try {
      ...
      const res = await fetch('https://rentify-server-iaby.onrender.com/api/auth/signout');
      const data = await res.json();
      if (data.success === false) {
        dispatch(deleteUserFailure(data.message));
        return;
  ...
  const handleShowListings = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://rentify-server-iaby.onrender.com/api/user/listings/${currentUser._id}`);
  ...
  const handleListingDelete = async (listingId) => {
    try {//listingId is id for which we wanted to delete the listing
      const res = await fetch(`https://rentify-server-iaby.onrender.com/api/listing/delete/${listingId}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
